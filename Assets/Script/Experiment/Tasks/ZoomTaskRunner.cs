...
public class ZoomTaskRunner : MonoBehaviour
{
    ...
    {
        // ✅ Zoom 시작 전에 "카메라 기본값"에서 출발 (VR 모드 제외)
        if (!isVRMode && mainCamera != null)
        {
        ...
        // ✅ Zoom 중에는 Follow 일시 정지
        if (camFollow != null)
        {
            camFollow.paused = true;
        }
    ...
    public void RestoreCameraBaseline(bool restoreFov)
    {
        ...
        // ✅ Zoom 끝나면 Follow 다시 활성화 (VR에서도!)
        if (camFollow != null)
        {
            camFollow.paused = false;
            camFollow.enabled = true;
            camFollow.ResetToStart();
        }

        // VR 모드에서는 카메라 위치/회전 직접 변경하지 않음 (XR Rig이 관리)
        if (!isVRMode && mainCamera != null)
        {
            mainCamera.transform.position = camStartPos;
            mainCamera.transform.rotation = camStartRot;
    ...
    /// <summary>
    /// 타겟공을 숨기고 크기를 원래대로 초기화
    /// </summary>
    public void ResetTargetBall()
    {
        if (targetBall != null)
        {
            targetBall.localScale = Vector3.one * normalScale;
            targetBall.gameObject.SetActive(false);
            Debug.Log("[ZoomTaskRunner] TargetBall 크기 초기화 및 숨김");
        }
    }
    ...
    {
        // VR 모드가 아닐 때만 FOV 줌 사용
        if (!isVRMode && zoomByFOV && mainCamera != null)
        {
        ...
        // VR 모드가 아닐 때만 Dolly 줌 사용
        if (!isVRMode && zoomByCameraDolly && mainCamera != null)
        {
        ...
        if (zoomByTargetScale && targetBall != null)
        {
            ...
            Debug.Log($"[ZoomTaskRunner] Scale Zoom: {from} → {to}");
