...
public static class Global
{
    ...
    // Original robot paths (may need to be updated for your scene)
    ...
    /// <summary>
    /// Get the tooltip path (for compatibility)
    /// </summary>
    ...
    /// <summary>
    /// Get the second tooltip path (for compatibility)
    /// </summary>
    public static string GetTooltipPath2()
    {
        return tooltip_path2;
    }
    ...
    public static void Arrow(Vector3 from, Vector3 to, Color color) {
        ...
        for (int i = 0; i < coneResolution; i++) {
            ...
            if (i == coneResolution - 1) 
                Debug.DrawLine(tipcenter + tipBasePoints[i], tipcenter + tipBasePoints[0], color);
            else 
                Debug.DrawLine(tipcenter + tipBasePoints[i], tipcenter + tipBasePoints[i + 1], color);
        }
    }
    ...
    public static void DebugOnHRSV(string text) {
        try {
            var debugTextR = GameObject.Find("/Text/CanvasDebug/DebugText");
            var debugTextL = GameObject.Find("/Text/CanvasDebugL/DebugTextL");
            
            if (debugTextR != null) {
                var textComp = debugTextR.GetComponent<UnityEngine.UI.Text>();
                if (textComp != null) textComp.text += text + "\n";
            }
            if (debugTextL != null) {
                var textComp = debugTextL.GetComponent<UnityEngine.UI.Text>();
                if (textComp != null) textComp.text += text + "\n";
            }
        } catch {
            // Silently ignore if UI elements don't exist
        }
    }
}
